...
/**
 * @brief This function applies the Castling's move to the given chess board board.
 * 
 * @param board The current board to be transitioned
 * @return A new board if the transition is valid, or std::nullopt otherwise
 */
std::optional<ChessBoard> Castling::operator()(const ChessBoard& board) const {
    
    // check if castling is allowed in the first place 
    if (!board.castling(_color, _isLeft)) {
        return std::nullopt;
    }

    // if the king is in check, castling is impossible
    if (!CheckTerminal::kingIsChecked(board, _color).empty()) {
        return std::nullopt;
    }
    ...
    internal::utility::writeData(boardData, newKingPos, board.getPiece(kingPos));

    internal::utility::writeData(boardData, rookPos, ChessPiece::NONE);
    internal::utility::writeData(boardData, newKingPos + mv, board.getPiece(rookPos));
