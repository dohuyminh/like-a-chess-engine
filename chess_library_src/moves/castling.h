 ...
 * but if the transformation is performed on a game board that does not allow castling, or there are pieces between 
 * the rook and the king, it returns ```std::nullopt```.  
 */
class Castling final : public ChessMove {
public:
    Castling(Color color, bool isLeft) noexcept;

    [[nodiscard]] inline bool moveLeft() const noexcept {
        return _isLeft;
    }
